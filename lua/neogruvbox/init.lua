local colors = require "neogruvbox.palette"
local config = require "neogruvbox.config"
local neogruvbox = {}
...
--- Apply user settings.
---@param values table
function neogruvbox.setup(values)
  setmetatable(config, { __index = vim.tbl_extend("force", config.defaults, values) })
end

--- Set the colorscheme.
function neogruvbox.colorscheme()
  if vim.version().minor < 8 then
    vim.notify(
      "Neovim 0.8+ is required for neogruvbox colorscheme",
      vim.log.levels.ERROR,
      { title = "neogruvbox colorscheme" }
    )
    return
  end
